...
def test_ws_requires_valid_token(starlette_client):
    ...
    with pytest.raises(Exception):
        with starlette_client.websocket_connect("/api/v1/ws/tasks/t1?token=invalid") as ws:
            ws.receive_text()


def test_ws_hitl_message_publishes_to_redis(starlette_client, test_user_token):
    ...
    with patch("app.routers.ws.hitl_coordinator.publish_response", AsyncMock()) as mock_pub:
        try:
            with starlette_client.websocket_connect(
                f"/api/v1/ws/tasks/t1?token={test_user_token}"
            ) as ws:
...
def test_ws_invalid_json_returns_error(starlette_client, test_user_token):
    """Sending invalid JSON returns an error message without disconnecting."""
    with starlette_client.websocket_connect(
        f"/api/v1/ws/tasks/t2?token={test_user_token}"
    ) as ws:
...
def test_ws_non_hitl_message_is_ignored(starlette_client, test_user_token):
    """Unknown message type is silently ignored (no ack, no error)."""
    with patch("app.routers.ws.hitl_coordinator.publish_response", AsyncMock()) as mock_pub:
        with starlette_client.websocket_connect(
            f"/api/v1/ws/tasks/t3?token={test_user_token}"
        ) as ws:
            ...
            data = json.loads(response)
            assert data["type"] == "ack"
        mock_pub.assert_called_once_with(task_id="t3", response="still alive")
